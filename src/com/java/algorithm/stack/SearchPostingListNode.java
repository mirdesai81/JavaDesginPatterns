package com.java.algorithm.stack;

import java.util.Deque;
import java.util.LinkedList;

public class SearchPostingListNode {
    public static void searchPostingsList(PostingListNode L) {
        Deque<PostingListNode> s = new LinkedList<>();
        s.addFirst(L);
        int order = 0;
        while(!s.isEmpty()) {
            PostingListNode curr = s.removeFirst();
            // not visited
            if(curr != null && curr.getOrder() == -1) {
                curr.setOrder(order++);

                // Stack is last-in, first -out, and we want to process
                // the jump node first, so push next, then push jump.
                s.addFirst(curr.getNext());
                s.addFirst(curr.getJump());
            }
        }
    }

    public static void searchPostingsListRecurrsive(PostingListNode L) {
        setJumpOrderHelper(L,0);
    }

    private static int setJumpOrderHelper(PostingListNode L, int order) {
        if (L != null && L.getOrder() == -1) {
            L.setOrder(order++);
            order = setJumpOrderHelper(L.getJump(), order);
            order = setJumpOrderHelper(L.getNext(), order);
        }

        return order;
    }

    public static void main(String[] args) {
        PostingListNode L = null, curr = null;
        // Build a linked list L->1->2->3->4->5->nullptr.
        for (int i = 0; i < 5; ++i) {
            PostingListNode temp = new PostingListNode(-1, null, null);
            if (curr != null) {
                curr.setNext(temp);
                curr = temp;
            } else {
                curr = L = temp;
            }
        }

        L.setJump(null); // no jump from 1
        L.getNext().setJump(L.getNext().getNext().getNext()); // 2's jump points to 4
        L.getNext().getNext().setJump(L); // 3's jump points to 1
        L.getNext().getNext().getNext().setJump(null); // no jump from 4
        L.getNext().getNext().getNext().getNext()
                .setJump(L.getNext().getNext().getNext().getNext()); // 5's jump points to 5
        PostingListNode temp = L;
        searchPostingsList(L);
    }
}
